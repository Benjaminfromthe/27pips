...
# Maps lesson DB titles to i18n keys (title_key, content_key)
# Allows lesson content + titles to be served in the active language
LESSON_KEY_MAP = {
...
def _make_t():
    """Build a t() resolver from the current session language."""
...
def _resolve_lesson(lesson: dict, t) -> dict:
    """Overwrite title + content with translated values when a key mapping exists."""
    keys = LESSON_KEY_MAP.get(lesson.get('title', ''))
    if keys:
        lesson['title']   = t(keys[0])
        lesson['content'] = t(keys[1])
    return lesson


def _resolve_course(course: dict, t) -> dict:
    """Overwrite title + description with translated values when a key mapping exists."""
    keys = COURSE_KEY_MAP.get(course.get('slug', ''))
    if keys:
        course['title']       = t(keys[0])
        course['description'] = t(keys[1])
    return course
...
def education_home():
    ...
    courses = [dict(c) for c in db.execute('SELECT * FROM courses ORDER BY order_number').fetchall()]
    ...
    for course in courses:
        ...
        for l in all_lessons:
            ...
            # Translate title only (content not rendered on listing page)
...
def get_lessons():
    db   = get_db()
    rows = db.execute(
        'SELECT id, title, course_id, order_number, duration_minutes FROM lessons ORDER BY course_id, order_number'
    ).fetchall()
    db.close()
